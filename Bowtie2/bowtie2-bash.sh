...
OUTFILE=$(jq --raw-output '. | ."Bowtie2-outputfile" // empty' ${JSON} | tr '\n' ' ')
...
then
	...
	CMD2="bowtie2 --rg-id test1 --rg SM:test1 -x ${TMP_DIR}/bowtie2-index -1 ${READ1} -2 ${READ2} -S ${TMP_DIR}/${OUTFILE}"
elif [ ${TYPE} == "single" ]
then
	READ=$(jq --raw-output '. | ."read"."read-files"[]."read" // empty' ${JSON} | tr '\n' ' ')
	CMD2="bowtie2 --rg-id test1 --rg SM:test1 -x ${TMP_DIR}/bowtie2-index -U ${READ} -S ${TMP_DIR}/${OUTFILE}"
fi
