...
OUTFILE=$(jq --raw-output '. | ."GATK-HaplotypeCaller-outputfile" // empty' ${JSON} | tr '\n' ' ')

CMD1="java -jar /usr/local/bin/GenomeAnalysisTK.jar -R ${REF} -T HaplotypeCaller -I ${BAM} -o ${TMP_DIR}/${OUTFILE}"
